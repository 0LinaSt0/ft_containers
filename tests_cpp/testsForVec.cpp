...
namespace ft{
	void	checkVecConstructors(void){
		...
		long	ft_time = 0, std_time = 0;
		{
			...
			timer.start();
			vector<std::string>		my_vec(_factor_, "Hola"); 
			ft_time = timer.stop();

			timer.start();
			std::vector<std::string>	orig_vec(_factor_, "Hola");
			std_time = timer.stop();
...

		{
			...
			ft::vector<char>	my_vec(_factor_, 'a');
			timer.start();
			ft::vector<char>	my_vec1(my_vec.begin(),(my_vec.end() - 1));
			ft_time = timer.stop();

			std::vector<char>	orig_vec(_factor_, 'a');
			timer.start();
...

		{
			...
			ft::vector<int>	my_vec(_factor_, 5);
			timer.start();
			ft::vector<int>	my_vec1(my_vec);
			ft_time = timer.stop();

			std::vector<int>	orig_vec(_factor_, 5);
			timer.start();
	...
	void	vectorChecker(valueType type){
		...
		// } else if (type == INT) {
			ft::vector<int>		my_vec_int(_factor_);
			std::vector<int>	orig_vec_int(_factor_);
